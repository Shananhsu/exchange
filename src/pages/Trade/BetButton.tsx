...
const BetButton = ({ commodity, range, handlBuy }: Props) => {
    const [touchWinL, touchWinR] = useMemo(() => {
        ...
        return [
            (commodity.lastclose * (1 + Number(range.winpercent) / 100)).toString().toDigital(commodity.floatdigit),
            (commodity.lastclose * (1 - Number(range.winpercent) / 100)).toString().toDigital(commodity.floatdigit)
        ];
    }, [commodity, range]);
    ...
    const [touchLoseL, touchLoseR] = useMemo(() => {
        if (!commodity) return [0, 0];
        return [
            (commodity.lastclose * (1 - Number(range.losepercent) / 100)).toString().toDigital(commodity.floatdigit),
            (commodity.lastclose * (1 + Number(range.losepercent) / 100)).toString().toDigital(commodity.floatdigit)
        ];
    }, [commodity, range]);
    return (
        <div className="mt-5 flex justify-center text-white">
            <div className="w-1/2 text-center">
                <button className="w-1/2 rounded-lg bg-green-500 py-2 text-2xl font-black" onClick={() => handlBuy(1)}>
                    看漲
                </button>
            ...
            <div className="w-1/2 text-center">
                <button className="w-1/2 rounded-lg bg-lose py-2 text-2xl font-black" onClick={() => handlBuy(-1)}>
                    看跌
                </button>
                <div className="mt-2 text-xs">
