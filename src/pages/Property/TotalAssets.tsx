...
const TotalAssets = ({ balance, totalasset, profit }: Props) => {
    ...
    return (
        <div className="font-black text-white">
            <div className="mx-auto h-56 w-10/12 rounded-lg bg-arerBgc pl-5">
                <h1 className="pt-3 text-lg">總資產</h1>
                <h1 className="mt-2 text-lg">{totalasset.toLocaleString()}</h1>
                <div className="mt-2 flex">
                    <p className="w-3/6">可用餘額</p>
                    <p>今日盈虧</p>
                </div>
                <div className="mt-2 flex">
                    <p className="w-3/6">{balance.toLocaleString()}</p>
                    <p className={color}>{prefix}{profit}</p>
                </div>
